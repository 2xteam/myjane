...
for (const app of APPS) {
  ...
  // ── G. 포인트 요소를 남발했나 ─────────────────────
  // 형광 밑줄은 **드물어야 작동한다.** 헤드라인마다 칠하면 강조가 사라진다.
  //
  // ⚠️ sheet--point 는 여기서 세지 않는다. 한때 "한 화면에 2개" 로 막았는데,
  // 2026-09-07 에 의미가 바뀌었다 — 강조 장식이 아니라 **"이 시트는 안에 카드를
  // 담고 있다" 는 구조 신호**다. 그런 시트가 여섯 개면 여섯 개 다 붙는 게 맞다.
  // 개수로 막으면 오히려 규칙을 지킨 페이지가 실패한다. 되살리지 말 것.
  ...
  for (const f of codeFiles) {
    if (allowed(f)) continue;
    const t = await readFile(f, "utf8");
    for (const [cls, cap, label] of POINT_CAPS) {
      // className 안의 클래스 목록에서만 센다 — 주석·문자열은 세지 않는다
      let n = 0;
      for (const m of t.matchAll(/className=\{?["'`]([^"'`]*)["'`]/g)) {
        if (m[1].split(/\s+/).includes(cls)) n++;
      }
      if (n > cap) add(app, "G 포인트 요소 남발", f, `${label} ${n}개 — 한 화면에 ${cap}개까지`);
    }
  }
...
const RULES = {
  ...
  "G 포인트 요소 남발": "형광 밑줄은 한 화면에 한 군데다. 라운딩(sheet--point)은 구조 신호라 세지 않는다",
};
